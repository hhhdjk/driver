...
static void timer_fn(unsigned long arg)
{
    ...
    printk("key %s , %s\r\n", pdata->name , status ? "released" : "pressed" );
}

static void Tsaklet_handler(unsigned long arg)
{
	time.data = arg;
...
/* 
	static inline void tasklet_init(struct tasklet_struct *tasklet, 
									...
									unsigned long data);
 */
static DECLARE_TASKLET(key_tasklet , Tsaklet_handler ,0 );

static irqreturn_t IRQ_handler(int irq, void *dev_id)
{
	spin_lock(&lock);
	key_tasklet.data = (unsigned long)dev_id;
	spin_unlock(&lock);
