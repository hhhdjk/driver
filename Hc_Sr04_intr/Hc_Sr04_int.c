...
ssize_t p_read (struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	time = wait_event_interruptible_timeout(r_wait , Flag_dat , HZ);
	if(time == 0){
		printk("timeout\r\n");
		irq_cnt = 0;
		return -EAGAIN;
	}
	printk("%s, Line at %d, irq cnt1 %d\r\n" , __FUNCTION__, __LINE__, irq_cnt1);
...
static irqreturn_t Hc_Sr04_irq(int irq, void *dev_id)
{
	int val , reg;
	irq_cnt++;
